...
impl ClientInstaller {
    ...
    pub(super) async fn download_file_force<R: Runtime>(
        ...
        if dest.exists() {
            if force {
                println!("Принудительное удаление старого файла: {:?}", dest);
                fs::remove_file(dest)?;
            } else if let Ok(metadata) = fs::metadata(dest) {
                let size = metadata.len();
                println!("Файл уже существует: {:?} ({} байт)", dest, size);

                // Если файл пустой или слишком маленький, удаляем и скачиваем заново
                if size < 1000 {
                    println!("Файл слишком маленький, удаляем и скачиваем заново");
                    fs::remove_file(dest)?;
                } else {
                    println!("Файл выглядит нормально, пропускаем скачивание");
                    return Ok(());
                }
            }
        }
